...
export default class HomeController {
    ...
    | WebPart Properties |
    =================*/
    private libraryname: string = null;
    private webabsoluteurl: string = null;
    ...
    constructor(private dataService: IDataService, private $window: ng.IWindowService, private $rootScope: ng.IRootScopeService, private $scope: ng.IScope) {
        ...
        =================*/
        $rootScope.$on('configurationChanged', (event: ng.IAngularEvent, args: { libraryname: string; webabsoluteurl: string }): void => {
            hctrl.init(args.libraryname, args.webabsoluteurl);
        });
    }
    ...
    | View Initialization |
    =================*/
    private init(libraryname: string, webabsoluteurl: string): void {
        ...
        if (libraryname != null && libraryname.length > 0 &&
            webabsoluteurl != null && webabsoluteurl.length > 0) {
            hctrl.libraryname = libraryname;
            hctrl.webabsoluteurl = webabsoluteurl;
    ...
    private handleVideoUpload(event?: any): void {
        ...
        try {
            ...
            hctrl.dataService.uploadFile(contents, fileName, hctrl.webabsoluteurl, hctrl.libraryname).then((itemId: string): void => {
    ...
    public handleFileUpload(event?: any): void {
        ...
        try {
            hctrl.dataService.uploadFile(fileBuffer, fileName, hctrl.webabsoluteurl, hctrl.libraryname).then((itemId: string): void => {
                hctrl.message = "File uploaded Id is : " + itemId + " with name " + fileName;
            }).catch((err): void => {
    ...
    public handleVideoRetry(event?: any): void {
        ...
        try {
            ...
            hctrl.init(hctrl.libraryname, hctrl.webabsoluteurl);
        }
        catch (err) {
